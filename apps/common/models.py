...
class ContentView(TimeStampedModel):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE,verbose_name=_("User"), related_name="content_views")
    viewer_ip = models.GenericIPAddressField(verbose_name=_("Viewer IP Address"), blank=True, null=True)
    ...
    @classmethod
    def record_view(cls,content_type:Any,object_id,user:Optional[User],viewer_ip:Optional[str]) -> None:
        content_type = ContentType.objects.get_for_model(content_type)
        try:
            view,created = cls.objects.get_or_create(
                content_type=content_type,
                object_id=object_id,
                defaults={"user":user,"viewer_ip":viewer_ip, 'last_view':timezone.now()},
            )
            if not created:
